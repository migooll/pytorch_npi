...
class FullAPI(NPIView):
    ...
    def __init__(self, sw, full_demo=False, robot=False, model=None, program_set=None):
...
class ProgramSet(FullAPI):
    def __init__(self):
        self.map = {}
        self.program_id = 0
        for program in self.programs:
            self.register(Program(program[0]))
